...
sd_webui_path = os.getcwd()
instant_mesh_path = os.path.join(sd_webui_path, 'extensions/sd-webui-InstantMesh')

print("==================")
print(instant_mesh_path)
...
def on_ui_tabs():
    with gr.Blocks(analytics_enabled=False) as InstantMesh:
            ...
            with gr.Column():
                ...
                with gr.Row():
                    with gr.Group():
                        ...
                        sample_seed = gr.Number(value=42,
                                                label="Seed  (Try a different value if the result is unsatisfying)",
                                                precision=0)
                ...
                with gr.Row(variant="panel"):
                    examples_path = os.path.join(instant_mesh_path, "examples")

                    gr.Examples(
                        examples=[
                            os.path.join(examples_path, img_name) for img_name in sorted(os.listdir(examples_path))
                        ],
                        inputs=[input_image],
                        label="Examples",
    ...
    return [(InstantMesh, "InstantMesh", "instant_mesh")]
